...
function client_portal_current_client_id(): int
{
    ...
    // Internal authenticated OPS users may pass client_id for staging/admin verification.
    // Anonymous visitors must use a portal invite/session and cannot enumerate clients.
...
{
    $rows = vendor_device_status_for_client($clientId, $vendorCode);
    $safe = [];
    foreach ($rows as $row) {
        $safe[] = [
...
{
    if (!$row) return 'Not included';
    $status = strtoupper((string)($row['status'] ?? ''));
    $label = strtoupper((string)($row['status_label'] ?? ''));
    if ($label === 'MISSING' || in_array($status, ['MISSING', 'OFFLINE', 'STALE'], true)) return 'Not reporting';
    if (in_array($status, ['ACTIVE', 'REPORTED', 'OK', 'SUCCESS'], true) || in_array($label, ['ACTIVE', 'ONLINE', 'PROTECTED', 'REPORTED BY COVE'], true)) return 'Protected';
    if (in_array($status, ['WARNING', 'DEGRADED', 'ERROR', 'FAILED'], true)) return 'Needs attention';
    return 'Unknown';
}
...
function client_portal_grouped_assets(int $clientId): array
{
    ...
    foreach (client_portal_vendor_rows($clientId) as $row) {
        ...
        $key = $assetId > 0 ? ('asset:' . $assetId) : ('host:' . client_portal_normalized_device_key($name));
        if ($key === 'host:') $key = 'row:' . count($assets);
        $assets[$key] ??= ['key' => $key, 'syncro_asset_id' => $assetId, 'device_name' => $name ?: 'Unassigned device', 'os_name' => '', 'username' => '', 'vendors' => []];
        if ($assetId > 0) $assets[$key]['syncro_asset_id'] = $assetId;
        foreach (['os_name', 'username'] as $field) if ($assets[$key][$field] === '' && $row[$field] !== '') $assets[$key][$field] = $row[$field];
...
function client_portal_asset_by_key(int $clientId, string $key): ?array
{
    foreach (client_portal_grouped_assets($clientId) as $asset) {
        if (hash_equals((string)$asset['key'], $key)) return $asset;
    }
    return null;
}
