...
/** Staging FN v0.2 import. Including this file has no database side effects. */
...
function field_tracker_staging(PDO $pdo): void
{
    if ((string)$pdo->query('SELECT DATABASE()')->fetchColumn() !== 'mjrmstlj_mittops_test') {
        throw new RuntimeException('Tracker import is staging-only; no Live changes are allowed.');
    }
}
