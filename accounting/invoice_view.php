    ...
    <div class="card" style="padding:16px;overflow:auto;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        ...
        <?php else: ?>
          ...
          <tr style="border-bottom:1px solid rgba(255,255,255,.06)">
            <td style="padding:10px 8px;"><a href="<?= accounting_h(BASE_URL) ?>/payments/view.php?id=<?= (int)$payment['payment_id'] ?>" style="color:#9bd0ff;text-decoration:none;font-weight:600;"><?= accounting_h((string)$payment['payment_date']) ?></a><div style="font-size:12px;opacity:.65;"><?= accounting_h((string)($payment['reference_number'] ?: '—')) ?></div><?php if (!empty($payment['processor_receipt_url'])): ?><div style="font-size:12px;margin-top:3px;"><a href="<?= accounting_h((string)$payment['processor_receipt_url']) ?>" target="_blank" rel="noopener" style="color:#9bd0ff;text-decoration:none;">Stripe receipt</a></div><?php endif; ?></td>
            <td class="status-cell" style="padding:10px 8px;"><?= accounting_payment_method_badge_html((string)$payment['payment_method']) ?><?php if (!empty($payment['processor_payment_method_label'])): ?><div style="font-size:12px;opacity:.65;margin-top:4px;"><?= accounting_h((string)$payment['processor_payment_method_label']) ?></div><?php endif; ?></td>
