...
$deleteSummary = client_delete_summary($clientId);
?>
<?php if ($flashMsg !== ''): ?><div class="flash-success"><?= htmlspecialchars($flashMsg) ?></div><?php endif; ?>
<div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
  <div><h1 style="margin:0;font-size:28px;"><?= htmlspecialchars((string)$client['legal_name']) ?></h1><div style="opacity:.74;">Client detail, contacts, locations, contracts, and linked billing.</div></div>
  <div style="display:flex;gap:10px;flex-wrap:wrap;align-items:center;">
    <a class="btn btn-secondary" style="width:auto;padding:10px 14px;" href="<?= htmlspecialchars(BASE_URL) ?>/clients/edit.php?client_id=<?= $clientId ?>">Edit company</a>
    <a class="btn btn-secondary" style="width:auto;padding:10px 14px;border-color:#7f1d1d;background:rgba(127,29,29,.16);color:#fecaca;" href="<?= htmlspecialchars(BASE_URL) ?>/clients/delete.php?client_id=<?= $clientId ?>">Delete company</a>
  </div>
</div>
...
<p><strong>Delete readiness:</strong> <?php if (!empty($deleteSummary['can_delete'])): ?>Can be deleted locally.<?php else: ?>Blocked by <?= htmlspecialchars(implode(', ', $deleteSummary['blocking'])) ?>.<?php endif; ?></p>
<hr>
