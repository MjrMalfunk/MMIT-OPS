...
// Tracker imports are staging-only. Keep the dictated closeout separate from
// the work order's normal notes by reading the immutable import export.
if (ops_is_staging_env() && strcasecmp((string)($wo['platform'] ?? ''), 'FieldNation') === 0) {
    try {
        ...
        // Old staging installs without tracker tables remain usable.
    }
}
...
<main class="page">
  <div class="topbar">
    ...
    <div class="actions">
      <?php if (ops_is_staging_env() && strcasecmp((string)$wo['platform'], 'FieldNation') === 0): ?>
