...
- MMIT onboarding acceptance stores the created ready/move ticket number on the asset in `MMIT Ready Move Ticket ID` (with `MMIT Auto Move Ticket ID` also recognized by the mover for compatibility).
- After successful post-move verification, the runner writes `MMIT Auto Move Result` and `MMIT Onboarding Completed At`, then reads the stored ready/move ticket ID from the asset and adds a completion note directly to that onboarding/auto-move ticket. The legacy open-ticket search is fallback only. It does not resolve or close the ticket; manual technician verification and closure are still required. If no stored ticket ID is present and fallback search cannot find the related ticket, the runner logs a warning and keeps the verified move successful.
