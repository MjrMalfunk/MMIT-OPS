...
function syncro_api_request(string $method, string $path, array $query = [], ?array $payload = null): array
{
    ...
    if ($method === 'POST' && in_array($path, ['tickets/7001/comments', 'tickets/7002/comments'], true)) {
        return ['ok' => true, 'status' => 201, 'data' => ['comment' => ['id' => $path === 'tickets/7001/comments' ? 91 : 92]]];
    }
...
smoke_assert(($workstation['ticket_id'] ?? null) === 7001, 'workstation ticket id extracted', $failed);
...
smoke_assert(count($workstationComments) === 1, 'workstation move comments on found ticket', $failed);
...
smoke_assert(($server['ticket_id'] ?? null) === 7002, 'server-disabled ticket id extracted', $failed);
...
smoke_assert(count($serverComments) === 1, 'server-disabled comments on found ticket', $failed);
