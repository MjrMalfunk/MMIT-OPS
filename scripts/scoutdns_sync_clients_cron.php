...
function scoutdns_sync_status(array $item): array {
    ...
    $last = scoutdns_sync_dt($item['lastSeen'] ?? $item['last_seen'] ?? $item['lastCheckin'] ?? $item['lastSyncAt'] ?? $item['last_sync_at'] ?? $item['updatedAt'] ?? null);
    if ($enabled === false || $enabled === 0 || $enabled === '0') return ['DISABLED', 'Disabled', false, $last];
    $agentStatus = scoutdns_sync_safe($item['agentStatus'] ?? $item['agent_status'] ?? '', 80);
    return ['REPORTED', $agentStatus !== '' ? $agentStatus : 'Reported by ScoutDNS', null, $last];
}
...
function scoutdns_sync_device_id(array $item, string $device): string { foreach (['id','deviceId','device_id','agentId','agent_id','roamingClientId','clientId','client_id'] as $k) { $v = scoutdns_sync_safe($item[$k] ?? ''); if ($v !== '') return $v; } return 'scoutdns-' . vendor_telemetry_normalize_device_key($device); }
function scoutdns_sync_single_link_fallback(array $links): ?array {
    return count($links) === 1 && scoutdns_sync_safe($links[0]['vendor_org_id'] ?? '') !== '' ? $links[0] : null;
}
function scoutdns_sync_unmapped_reason(string $orgId, array $links): ?string {
    if ($orgId !== '') return 'no_active_scoutdns_link';
    if (count($links) === 0) return 'no_active_scoutdns_link';
    if (count($links) > 1) return 'ambiguous_scoutdns_links';
    return 'no_vendor_org_id';
}
...
try {
    ...
    $linksByOrg = []; foreach ($links as $link) { $linkOrgId = scoutdns_sync_safe($link['vendor_org_id'] ?? ''); if ($linkOrgId !== '') $linksByOrg[$linkOrgId] = $link; }
    $singleFallbackLink = scoutdns_sync_single_link_fallback($links);
    ...
    foreach ($items as $item) {
        ...
        $reportedOrgId = scoutdns_sync_vendor_org_id($item);
        $mappingStrategy = 'vendor_org_id';
        $link = $reportedOrgId !== '' ? ($linksByOrg[$reportedOrgId] ?? null) : null;
        $orgId = $reportedOrgId;
        if ($reportedOrgId === '' && is_array($singleFallbackLink)) {
            $link = $singleFallbackLink;
            $orgId = scoutdns_sync_safe($singleFallbackLink['vendor_org_id'] ?? '');
            $mappingStrategy = 'single_active_link_fallback';
        }
        $unmappedReason = is_array($link) ? null : scoutdns_sync_unmapped_reason($reportedOrgId, $links);
        [$status, $label, $protected, $lastSeen] = scoutdns_sync_status($item);
        $summary = ['source_endpoint'=>'GET /getClients','vendor_org_id'=>$orgId ?: null,'has_username'=>scoutdns_sync_safe($item['username'] ?? $item['userName'] ?? '') !== '','reported_status'=>$status,'mapping_strategy'=>is_array($link) ? $mappingStrategy : 'unmapped'];
        echo json_encode(['vendor'=>'scoutdns','mapped_client_id'=>$link['client_id'] ?? null,'device_name'=>$device,'status'=>$status,'last_seen_at'=>$lastSeen,'unmapped_reason'=>$unmappedReason,'mapping_strategy'=>is_array($link) ? $mappingStrategy : null,'synced_at'=>vendor_telemetry_now(),'apply'=>$apply], JSON_UNESCAPED_SLASHES) . "\n";
        $result['devices_seen']++;
